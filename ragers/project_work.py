...
class ProjectWork:
    ...
    def initialize_team(self):
        ...
        try:
            for role in self.config['required_roles']:
                # TODO Specify names of agents to specialize in code, write, plan, etc.
                # Parse role definition (e.g., "Supervisor" -> "Supervisor", "Supervisor")
                name = role.strip()
                role_type = name  # Use the role name as the role type
                ...
                self.logger.info(f"Initialized {name} as {role_type}")
                
        except Exception as e:
            self.logger.error(f"Failed to initialize team: {str(e)}")
            raise
            
    def get_agents_by_role(self, role_type: str) -> Dict[str, Any]:
        """Get all agents of a specific role type"""
        return {
            name: agent['instance'] 
            for name, agent in self.agents.items() 
            if role_type in agent['role_type']
        }
    ...
    def run_meeting(self, phase: str) -> bool:
        ...
        try:
            ...
            # Get responses from each participant based on role types
            for role_type in self._get_phase_participants(phase):
                agents = self.get_agents_by_role(role_type)
                for name, agent in agents.items():
                    self.logger.info(f"Processing {name} ({role_type}) in {phase} meeting")
                    ...
                    self._log_chatgpt_interaction(f"{role_type}_{name}", prompt, response)
                    
                    self._log_conversation(phase, f"{role_type}_{name}", response)
                    
                    # Generate meeting document for all phases
                    if doc_template:
                        # Add the response to the context for the document
                        context['response'] = response
                        self.logger.debug(f"Context for document generation: {context}")
    ...
    def _generate_meeting_doc(self, phase: str, context: dict) -> None:
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": "You are a document generation assistant. Your task is to merge the template and response into a coherent document."},
            ...
            # Get merged content from ChatGPT
            merged_content = CallChatGPT.get_response("gpt-4o", messages)
